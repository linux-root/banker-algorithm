...
App.config(['$stateProvider', '$urlRouterProvider',
    function ($stateProvider, $urlRouterProvider) {
        ...
        $stateProvider
            ...
            .state('demoModel.list', {
                url: '/',
                templateUrl: './assets/views/demo-models/list.html',
                ...
                resolve: {
                    deps: ['$ocLazyLoad', function ($ocLazyLoad) {
                        return $ocLazyLoad.load({
                            ...
                            serie: true,
                            files: [
                                './assets/js/core/ngtable/ng-table.min.css'
                            ]
                        })
                    }]
                },
            ...
            .state('demoModel.create', {
                url: '/create',
                templateUrl: './assets/views/demo-models/edit.html',
                controller: 'ModelCreateEditCtrl',
                resolve: {
                    deps: ['$ocLazyLoad', function ($ocLazyLoad) {
                        return $ocLazyLoad.load({
                            insertBefore: '#css-bootstrap',
...
// Run our App
App.run(function($rootScope, uiHelpers, TokenProvider, authManager, $state) {
    ...
    var $theToken = JSON.parse(window.localStorage.getItem('__the_token'));
    if (typeof $theToken == 'object' && $theToken != null && typeof $theToken.refresh_token == 'string') {
        var $refresher = function () {
            TokenProvider.refresh().then(function (r) {
                window.localStorage.setItem('__the_token', JSON.stringify(r.data));
                window.setTimeout($refresher, r.data.expires_in * 1000);
            }, function () {
                window.localStorage.removeItem('__the_token');
                $state.go('loginPage');
            });
        };
        window.setTimeout($refresher, 0);
    }
});

// Application Main Controller
App.controller('AppCtrl', ['$scope', '$localStorage', '$window', '$rootScope', 'authManager', 'jwtHelper', 'TokenProvider', '$state',
    function ($scope, $localStorage, $window, $rootScope, authManager, jwtHelper, TokenProvider, $state) {
        // Template Settings
        $scope.oneui = {
        ...
        $rootScope.$on('$stateChangeStart', function (event, toState) {
            $scope.oneui.settings.sidebarOpen = authManager.isAuthenticated();
        });
        function initClient() {
            gapi.client.init({
                apiKey: 'AIzaSyBVIaOmjtNG2kTu7sdHIMimuLaWGPNw_Wc',
                discoveryDocs: ["https://people.googleapis.com/$discovery/rest?version=v1"],
                clientId: '306684131134-vlgl9ibaoo8fboo2o1dsrgv24h2ov648.apps.googleusercontent.com',
                scope: 'profile'
            }).then(function () {
                // Listen for sign-in state changes.
                // cho khi dang nhap xong thi thuc hien
                gapi.auth2.getAuthInstance().isSignedIn.listen($rootScope.updateSignInStatus);
                // Handle the initial sign-in state.
                // chay ngay khi load trang web xong
                $rootScope.updateSignInStatus(gapi.auth2.getAuthInstance().isSignedIn.get());
            });
        }
        $rootScope.updateSignInStatus = function (isSignedIn) {
            if (isSignedIn) {
                // id_token from Google
                var GToken = gapi.auth2.getAuthInstance().currentUser.get().getAuthResponse().id_token;
                TokenProvider.grant(GToken).then(function (response) {
                    window.localStorage.setItem('__the_token', JSON.stringify(response.data.data));
                    var $theToken = JSON.parse(window.localStorage.getItem('__the_token'));
                    if (typeof $theToken == "object" && $theToken != null) {
                        $rootScope.userData = jwtHelper.decodeToken($theToken.access_token);
                    }
                    $state.go('dashboard');
                });
            } else {
...
// Header Controller
App.controller('HeaderCtrl', ['$scope', '$localStorage', '$window', 'TokenProvider', '$state', '$rootScope',
    function ($scope, $localStorage, $window, TokenProvider, $state, $rootScope) {
        // When view content is loaded
        $scope.$on('$includeContentLoaded', function () {
