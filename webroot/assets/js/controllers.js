...
App.controller('LoginCtrl', ['$scope','$rootScope', 'TokenProvider', '$state', function ($scope, $rootScope, TokenProvider, $state) {
...
/* Controller for Models List*/

App.controller('ModelsListCtrl', ['$scope', '$state', 'NgTableParams', 'DemoModel', function ($scope, $state, NgTableParams, DemoModel) {
    var demoModels = DemoModel.query(function () {
        demoModels.forEach(function (e, i) {
            e.index = i;
        });
    ...
    $scope.deleteModel = function (modelID) {
        var theModel = new DemoModel({database_id: modelID});
        console.log(theModel);
        theModel.$delete(function (theModel) {
            if (typeof theModel.database_id != "undefined") {
                $state.reload();
            } else {
...
/* Controller for Create/Edit Models */

App.controller('ModelCreateEditCtrl', ['$scope', '$state', '$resource', 'DemoModel', '$stateParams', 'ApiQuery', function ($scope, $state, $resource, DemoModel, $stateParams) {
    $scope.isEditing = false;
    if ($stateParams.modelID != undefined) {
        DemoModel.get({database_id: $stateParams.modelID}, function (theModel) {
            $scope.theModel = theModel;
        });
        $scope.isEditing = true;
    } else {
        $scope.theModel = new DemoModel();
    }
    $scope.submitModelForm = function () {
        if (!jQuery('.js-validation-material').valid())
            return false;
        if ($scope.theModel.database_id) {
            $scope.theModel.$update(function () {
                $state.go('demoModel.list');
            });
        } else {
            $scope.theModel.$save(function () {
                $state.go('demoModel.list');
            });
        }
    };
        ...
        jQuery('.js-validation-material').validate({
            ...
            messages: {
                ...
                'number': {
                    ...
                    min: 'Please make it greater than 0'
                }
            },
